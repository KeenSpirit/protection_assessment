...
#         safe_set_cell(worksheet, 'A18', f'Minimum phase-ground fault resistance for overhead lines set to {oh_z} ohms')
#         safe_set_cell(worksheet, 'A19',
#                       f'Minimum phase-ground fault resistance for underground lines set to {ug_z} ohms')
#         safe_set_cell(worksheet, 'A21', 'External Grid Data:')
#
#         i = 0
...
#             df_clean[col] = df_clean[col].astype(str).apply(clean_string_value)
#
#     # Clean column names
...
#         device_values = [
#             safe_round(device.l_l_volts),
#             safe_round(device.phases),
#             safe_round(device.ds_capacity),
#             safe_round(device.max_fl_ph),
#             safe_round(device.max_fl_pg),
#             safe_round(device.min_fl_ph),
#             safe_round(device.min_fl_pg),
#             str(device.max_ds_tr) if hasattr(device, 'max_ds_tr') else '',
#             safe_round(device.max_tr_size),
#             safe_round(device.tr_max_ph),
#             safe_round(device.tr_max_pg),
...
#     """
#     Safely round a value, handling None, inf, and other edge cases
#     """
#     try:
...
#         device_name = str(device.object.loc_name) if hasattr(device, 'object') and hasattr(device.object,
#                                                                                            'loc_name') else 'Unknown Device'
#         sect_terms = getattr(device, 'sect_terms', [])
#
#         df = pd.DataFrame({
#             device_name: [
#                 str(term.object.loc_name) if hasattr(term, 'object') and hasattr(term.object, 'loc_name') else str(term)
#                 for term in sect_terms],
#             # Data
#             'Construction': [str(getattr(term, 'constr', '')) for term in sect_terms],
#             'L-L Voltage': [safe_round(getattr(term, 'l_l_volts')) for term in sect_terms],
#             'No. Phases': [safe_round(getattr(term, 'phases')) for term in sect_terms],
#             # FAULT LEVELS
#             'Max PG fault': [safe_round(getattr(term, 'max_fl_pg')) for term in sect_terms],
#             'Max PH fault': [safe_round(getattr(term, 'max_fl_ph')) for term in sect_terms],
#             'Min PG fault': [
#                 safe_round(fault_impedance.term_pg_fl(region, term)) if hasattr(fault_impedance, 'term_pg_fl') else ''
#                 for term in sect_terms],
#             'Min 2P fault': [safe_round(getattr(term, 'min_fl_ph')) for term in sect_terms],
#             # PICKUPS
...
#
#         # Safely handle transformer data
#         try:
#             tr_dict = {str(load.term.loc_name): safe_round(load.load_kva) for load in device.sect_loads if
#                        hasattr(load, 'term') and hasattr(load.term, 'loc_name') and hasattr(load, 'load_kva')}
#             df_sorted['Tfmr Size (kVA)'] = df_sorted[device_name].map(tr_dict).fillna('')
#         except:
#             # If mapping fails, leave the column empty
#             pass
#
...
def format_study_results(devices):
    ...
    for device in devices:
        ...
        device_values = [
            ...
            str(device.max_ds_tr) if hasattr(device, 'max_ds_tr') else '',
            safe_numeric(device.max_tr_size),
            safe_numeric(device.tr_max_ph),
            safe_numeric(device.tr_max_pg),
            ', '.join(ds_device_names),
            ', '.join(us_device_names),
        ]
