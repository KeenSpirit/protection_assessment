...
Classes:
    Tfmr: Transformer/load dataclass

Functions:
    initialise_load_dataclass: Create Tfmr from PowerFactory element
"""
...
class Tfmr:
    ...
    The Tfmr dataclass is used for both individual distribution
    transformers (ElmTr2 in regional models) and aggregated load
    representations (ElmLod in SEQ models). It captures the information
    needed for fuse selection and protection coordination.
    ...
    Example:
        ...
        >>> print(f"Transformer: {tfmr.load_kva}kVA")
...
def initialise_load_dataclass(
    ...
    Handles both ElmLod (aggregated load) and ElmTr2 (transformer)
    objects, extracting the appropriate terminal reference and capacity
    value.
    ...
    Returns:
        Initialized Tfmr dataclass. Returns an empty Tfmr if load is
        None or not a recognized type.
